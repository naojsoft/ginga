...
class Expander(ContainerBase):
    ...
    def __init__(self, title=None, notoggle=False):
        ...
        self.toggle = None
        if not notoggle:
            toggle = ToggleButton(label=title)
            self.toggle = toggle
...
class GridBox(ContainerBase):
    def __init__(self, rows=1, columns=1):
        ...
        w = Gtk.Grid(rows=rows, columns=columns)
        self.widget = w
        self.tbl = {}
        self.num_rows = rows
            ...
            for i in range(self.num_rows - 1, index - 1, -1):
                for j in range(self.num_cols):
                    ...
                    if key in self.tbl:
                        ...
                        # move actual widget down in GtkGrid
                        w = child.get_widget()
                        self._remove(w)
                        row, col = i + 1, j
                        self.widget.attach(w, col, col + 1, row, row + 1,
            ...
            for i in range(index + 1, self.num_rows):
                for j in range(self.num_cols):
                    ...
                    if key in self.tbl:
                        child = self.tbl.pop(key)
                        self.tbl[(i - 1, j)] = child
                        # move actual widget up in GtkGrid
                        w = child.get_widget()
                        self._remove(w)
                        row, col = i - 1, j
                        self.widget.attach(w, col, col + 1, row, row + 1,
...
class Toolbar(ContainerBase):
    ...
    def add_action(self, text, toggle=False, iconpath=None, iconsize=None):
        if toggle:
            child = ToggleButton(label=text)
        else:
            child = Button(label=text)
...
class Application(Callback.Callbacks):
    ...
    def add_window(self, window, wid=None):
        self._gtkapp.add_window(window)
        if wid is None:
...
def build_info(captions, orientation='vertical'):
    ...
    table = Gtk.Grid(rows=numrows, columns=numcols)
